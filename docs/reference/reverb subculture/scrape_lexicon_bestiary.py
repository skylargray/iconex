#!/usr/bin/env python3
"""
Scrape the Gearspace "Lexicon Reverbs: A Brief Bestiary" thread and convert to markdown.

Usage:
    pip install requests beautifulsoup4
    python3 scrape_lexicon_bestiary.py

Output: lexicon-bestiary-pages-NN-NN.md (chunked into 5-page files)

Thread: https://gearspace.com/board/high-end/362930-lexicon-reverbs-brief-bestiary.html
"""
...
BASE_URL = "https://gearspace.com/board/high-end/362930-lexicon-reverbs-brief-bestiary"
TOTAL_PAGES = 14
...
def warm_up_session(session):
    """Hit the homepage first so we look like a real browser session."""
    print("Warming up session via homepage...")
    try:
        session.get('https://gearspace.com/', headers=HEADERS, timeout=30)
        time.sleep(2)
        session.get('https://gearspace.com/board/high-end/', headers=HEADERS, timeout=30)
        time.sleep(2)
    except Exception as e:
        print(f"  Warning: warm-up request failed ({e}), continuing anyway...")
...
def fetch_page(page_num, session):
    ...
    # Sec-Fetch-Site changes once we're navigating within the site
    headers = dict(HEADERS)
    headers['Sec-Fetch-Site'] = 'same-origin'
    headers['Referer'] = 'https://gearspace.com/board/high-end/'
...
def write_header():
    return """# Lexicon Reverbs: A Brief Bestiary -- Gearspace High End Thread
...
def main():
    ...
    print(f"Scraping Lexicon Bestiary thread, pages {start_page}-{end_page}")
