...
|---|---|---|---|---|---|
| MC | T&C | clock | PLL master clock 30.72 MHz | high | MC=30.72 MHz, period 32.55 ns = exactly one MS, so 1 pulse per MS (§3.5). ORIGIN (T&C 060-02475 sh.1, netlist 6T): generated by the PLL (tc_U26, omitted) and tapped at tc_U40.pin11 = the clock-driver output. MC clocks the MS generator tc_U39 (74F374 shift-reg) + tc_U56 (74S163, gives MS3/MS4); AS generator = tc_U23 (74LS175, clk ARUCK). This is the model's master-clock input. |
...
> ARU MAC pipeline across THREE microinstructions (i1,i2,i3); each AS = 3 MS = 97.66 ns. Owner-corrected placement 2026-06-27: operand read RA1/RA0 spans i1.AS1->i2.AS0; LOAD@i1.AS2 then SHIFT-right @i2.AS0 & i2.AS1; the operand weights (SHIFT REG), Booth coeff (M1,M0) and partial products all sit in i2.AS0-AS2; CSIGN valid i2.AS1->i3.AS0; the accumulate (0->±PP1->±(PP1+PP2)) runs i2.AS1->i3.AS0 (one AS behind the partial products = the pipeline register); ZERO/ and XFER CK pulse low at i2.AS0 (ZERO/ clears the accumulator so it reads 0 at i2.AS1; XFER CK lags ZERO/ ~20 ns). Shows the actions of one microinstruction's bits (pipelined/overlapped with neighbours). | 0027 per-pin verification: this pipeline placement is CONFIRMED at hardware pins by the ARU signature tables. Two sampling facts the tables measured: the HP analyzer samples LATE in each ARUCK cycle (both 74F283 ripple chains have SETTLED to the current computation at the sample instant) while the registers still capture the PREVIOUS sums at their earlier clock edges — the one-AS pipeline lag shown here is therefore pinned from both sides. All DAB-domain values in this figure are ACTIVE-LOW (the complement domain, session 0027).
...
| S1,S0 | T&C (fixed schedule) | control | fixed per-AS schedule | high | 74194 mode: LOAD@i1.AS2, SHIFT-right@i2.AS0 & i2.AS1 (owner-read) 0027: mode-line levels verified at the shifter pins by the signature tables (S0 reads all-high at the sample instants; S1 = the per-AS load pattern). |
...
| PARTIAL PRODUCT REG | ARU | data | Booth partial products | med | i2.AS0-AS2 (NOT explicitly owner-stated; kept one AS ahead of the accumulate = the pipeline register. CONFIRM) 0027: CONFIRMED (was 'CONFIRM') — PR Q pins verified per-pin; the D pins (front-end adder sums) sample as the LIVE phase, the Q pins as the latched phase — the register boundary is visible in the tables. |
| CSIGN | T&C tc_U20 JK-FF | control | tc_U20 (74S112) JK-FF output (NOT a stored microword bit): ARUCKE/-clocked, AS0-gated fn of MI23 | high | valid i2.AS1 -> i3.AS0 (owner-read). ORIGIN (netlist 3.10/G3R): CSIGN/ = tc_U20.pin9 (2Q), a JK FF clocked by ARUCKE/, J/K driven AS0-gated from MI23 (via tc_U18.QD->tc_U19.Q4->tc_U34 g3/g4). A faithful model must replicate the JK toggle, NOT read CSIGN as a microword bit (the prior 'lane-2 bit 7' belief was wrong). -> ARU aru_U2.pin11/13 (sub-ctrl, netlist 4F.8). 0027: the JK window (holds w[n-1]'s MI23 from the slot-3 edge) is verified at the aru_U2 pins (CSIGN/ = C356/U804 streams); rail = inv(CSIGN/), cin = inv(rail) confirmed as complementary streams at U2.5/6/10/12. |
| ACCUMULATOR | ARU | data | running sum | high | accumulate i2.AS1 -> i3.AS0 (owner-corrected: shifted right one cell). ZERO/ clears it at i2.AS0 so it reads 0 at i2.AS1 0027: verified per-pin, with a pin-role correction: the '163 Q pins pair own-letter (QA pin14 = the nibble's bit 3 .. QD pin11 = bit 0; netlist 4.1 corrected). The sat-mux clamp feeds the accumulator D inputs (a saturating group converges to the +/-2^18 rail; diag-3's tail proves the path). |
| ZERO/ | T&C tc_U48 -> ARU | strobe | microword ZERO-gate bit (MI25), gated at tc_U48 | high | active-LOW: low pulse at i2.AS0 (clears accumulator), high again at i2.AS1 (owner-read). ORIGIN (netlist 2T.3): ZERO/ = tc_U48.pin11 (74S00 g4) = NAND(tc_U19.Q6 [=MI25 ZERO-gate], AS0) -> ARU accumulator-reg /CLR aru_U45-49.pin1 (netlist 4.9). 0027: capture-before-clear verified per-pin (the clear lands one ARUCKE/ edge after the XFER capture). |
| XFER CK | T&C tc_U36 -> ARU | strobe | microword XFER bit (MI24), gated at tc_U36 | high | active-LOW: same low pulse at i2.AS0 as ZERO/ but ~20 ns lag behind it (owner-read). Latches ACC -> result register. ORIGIN (netlist 6T.4): XFER CK = tc_U36.pin6 (74S10 g2) = NAND(AS0, XFER [=tc_U19.Q5, MI24], tc_U25.pin9) -> ARU result-reg CP aru_U43/U44.pin11 (netlist 4.8). 0027: the result register D inputs are on the PP bus (0024) and the transfer is a pure PP3..18 bit tap — truncation of the complement-domain value; no rounding hardware. Chips: U43 = high byte, U44 = low byte (0027 correction). |

## fig33 — DMEM timing (DRAM access strobes)
*Source figure:* `fig-3.3-DMEM-timing.png` · *round-trip render:* `fig33.png`

> DMEM DRAM access within one microinstruction (MS grid). GRID PIXEL-CALIBRATED 2026-06-27: MS0 starts x=430, 81.1 px/MS, anchored to the printed MS-digit centers (MSO center=468). ALL signals owner-confirmed on this grid 2026-06-27 - the strobes (RAS//CAS//DAB WSTB//MEMW//DAB RSTB/RESET/), ROW SEL, and the OFST//ADDRESS/DOUT/DIN/CPC valid-windows (auto-extraction had been unreliable - annotation arrows cross the thin strobe traces - so they were read off by the owner). Per §3.6: DMEM strobes generated by delay-line dmem_U59 (DL6308, 5-tap 30ns/tap) from T&C signals MEMAC, DABSTB, MEMR. Address = CPC - OFST (2's-complement: CPC + complemented OFST/ + carry-in tied HIGH; adder U49/U50/U63/U64), row/col-muxed onto the 8 DRAM address lines by U18/U36. CPC = 16-bit counter U51/U65, +1 per sample. RAM outputs drive the DAB directly (no buffer). Notes: 'CAS/ falls only when MEMAC is high.' 'Critical path for DIN is XFER CK -> ARU result register.'
...
| RAS/ | DMEM (DL6308 seq) | strobe | timing | high | active-LOW; row-address strobe; <44 ns. ORIGIN (netlist 6D.2): RAS/ = dmem_U45.pin8 (74S08 g3), sequenced off the DL6308 (dmem_U59) taps; not gated by MEMAC (unlike CAS0/). |
...
| CAS/ | DMEM (DL6308 seq) | strobe | timing AND MEMAC | high | OWNER-CONFIRMED: active-LOW col-address strobe; HIGH MS0-3, falls@MS4, LOW MS4-8, rises@MS0 of next cycle (wraps). FALLS ONLY WHEN MEMAC HIGH. ORIGIN (netlist 6D.2): this is CAS0/ = dmem_U43.pin6 (74S10 g1); the MEMAC gate is dmem_U61B->dmem_U43A. (CAS1/ for the unpopulated lower bank dmem_U1-U16 is hard-disabled.) |
...
| DAB WSTB/ | T&C (= inv MS7) | strobe | timing (= NOT MS7) | high | READ: active-LOW; strobes read data onto DAB; <22 ns. ORIGIN (netlist 6T.4): DAB WSTB/ = tc_U37.pin6 = NOT(MS7) (so its low pulse = the MS7 cell); also the ARU regfile write-gate GW\ (aru_U29-32, netlist 4F.1). |
...
| DAB RSTB | T&C (tc_U20 FF1) | strobe | timing (tc_U20 JK FF, J=MS1/K=MS8) | high | WRITE: latches DIN from DAB; <12 ns. ORIGIN (netlist 6T.4): DAB RSTB = tc_U20.pin5 (74S112 FF1 1Q; J=MS1, K=MS8, CLK=tcCLKB) -> DMEM (netlist 6D.3) + the tc_U48/U49 device decode. Its complement DAB RSTB/ (tc_U20.pin6) clocks the PC counter + the offset latch + field regs + RESET FF (netlist 3.5/3.6/3.7/3.11). |
...
| RESET/ | DMEM local (dmem_U58A) | strobe | dmemRESET = inv(backplane RESET/), once per frame | high | OWNER RE-READ 2026-07-01: high at MS0 (<21 ns rise), high through MS5, falls from ~MS5 with a 35.5 ns transition (fully low ~MS7). RECONCILED with the traced generator: this fig-3.3 row depicts the DMEM-local dmemRESET (the DMEM sheet's local 'RESET' = inv(backplane RESET/), dmem_U58A) — the INVERSE of tc_U25 FF1's RESET/ (which is LOW MS0->MS6 on the post-reset step; MS6 force-clear via inv(MS6)). Its falling edge (~MS5-7) clocks the CPC ('393 counts on the fall), consistent with this figure's CPC row starting a new valid window at MS6. Occurs once per FRAME (the step after the reset microword), not every microinstruction. |
...
| CSIGN | fig34 | **NO (decode)** | tc_U20 (74S112) JK-FF output (NOT a stored microword bit): ARUCKE/-clocked, AS0-gated fn of MI23 |
| ACCUMULATOR | fig34 | **NO (decode)** | running sum |
| ZERO/ | fig34 | **NO (decode)** | microword ZERO-gate bit (MI25), gated at tc_U48 |
| XFER CK | fig34 | **NO (decode)** | microword XFER bit (MI24), gated at tc_U36 |
...
| DAB WSTB/ | fig33 | yes | timing (= NOT MS7) |
| MEMW/ | fig33 | yes | timing AND write |
| DAB RSTB | fig33 | yes | timing (tc_U20 JK FF, J=MS1/K=MS8) |
| DIN VALID | fig33 | **NO (decode)** | DRAM write data on the DAB (from ARU result reg) |
| RESET/ | fig33 | yes | dmemRESET = inv(backplane RESET/), once per frame |
